...
CHECKPOINT=${CHECKPOINT:-"$ROOT_DIR/runs/san_francisco/bireal/best.pt"}
...
exec "${PYTHON_BIN:-python}" -m experiments.san_francisco.train_bireal_flow \
  ...
  --workdir "${WORKDIR:-$ROOT_DIR/runs/san_francisco/bireal_lut}" \
  ...
  --pair-lut-parameterization "${PAIR_LUT_PARAMETERIZATION:-categorical}" \
  --epochs "${EPOCHS:-200}" \
